function [boxes_head] = detect_head(input, ...
  model, ...
  thresh, bbox, ...
  overlap, label, fid, id, maxsize)

% boxes_head = detect_head(input, model, thresh, bbox, overlap, label, fid, id, maxsize)
% Detect head objects in input using a model and a score threshold.
...
for level = interval+1:length(feat)
  ...
  % ftell lets us know where we are in the file
  ...
  for c = 1:model.numcomponents
    ...
    if ~latent
      ...
      for i = 1:length(I)
        x = X(i);
        y = Y(i);
        [x1, y1, x2, y2] = rootbox_head(x, y, scale, padx, pady, rsize{c});
        ...
        % tmp row i = [rootx, y, x, y, component, score]
        tmp(i,:) = [b c score(I(i))];
      end
      boxes_head = [boxes_head; tmp];
    end
  end
end
...
function [x1, y1, x2, y2] = rootbox_head(x, y, scale, padx, pady, rsize)
  % unlike in the original code, 
  % we add horizontal padding to the head detection bounding box
head_x1 = (x-padx)*scale+1;
head_y1 = (y-pady)*scale+1;
head_x2 = head_x1 + rsize(2)*scale - 1;
head_y2 = head_y1 + rsize(1)*scale - 1;
head_height = head_y2 - head_y1 + 1;
head_width = head_x2 - head_x1 + 1;
x1 = head_x1;  % - head_width/2;
x2 = head_x2;  % + head_width/2;
y1 = head_y1;
y2 = head_y2;  % + head_height*4;
% fprintf('head_y2: %d, head_height: %d, y2: %d\n', head_y2, head_height, y2);
