function [boxes1, boxes2] = pascal_test(cls, model, testset, suffix)
  ...
  disp(['Test file read in pascal_test: ', sprintf(VOCopts.imgsetpath, testset)]);
  disp(['Number of elements in ids in pascal_test: ', num2str(length(ids))]);
  % ids = textread(sprintf(VOCopts.imgsetpath, VOCopts.testset), '%s');
  
  % run detector in each image
  try
    load([cachedir cls '_just_person_boxes_' testset '_' suffix]);
  catch
    for i = 1:length(ids);
    ...
    save([cachedir cls '_just_person_boxes_' testset '_' suffix], 'boxes1', 'boxes2');
  end