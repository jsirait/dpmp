...
try
  load([cachedir cls 'on_coco_head_' testset '_' suffix]);
catch
  for i = 1:length(ids);
    ...
    boxes = detect_head(im, model, model.thresh);
    if ~isempty(boxes)
      b1 = boxes(:,[1 2 3 4 end]);
      % b1_head = boxes_head(:,[1 2 3 4 end]);
      % b1 = [b1; b1_head];
      b1 = clipboxes(im, b1);
      boxes1{i} = nms(b1, 0.5);
      if length(model.partfilters) > 0
        b2 = getboxes(model, boxes, true);
        b2 = clipboxes(im, b2);
        boxes2{i} = nms(b2, 0.5);
      else
        boxes2{i} = boxes1{i};
  ...
  save([cachedir cls 'on_coco_head_' testset '_' suffix], 'boxes1', 'boxes2');
end
